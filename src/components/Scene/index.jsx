...
import travTextureJpg from '../../assets/pictures/trav.jpg'
...
import ytPatchMorningJpg from '../../assets/pictures/morning.jpg'
import ytPatchNightJpg from '../../assets/pictures/Night.jpg'
import youtubeLogo from '../../assets/pictures/Youtube-Logo.png?url'
import resumePdf from '../../assets/resume/Yash_Kumar_Raut_resume.pdf?url'
...
import weeblyVideoMp4 from '../../assets/videos/Weebly.mp4'
import supplexVideoMp4 from '../../assets/videos/Supplex.mp4'
import gdocVideoMp4 from '../../assets/videos/GDoc.mp4'
...
function Scene() {    
    ...
    const travTexture = useTexture(travTextureJpg)
    const roomMorningTexture = useTexture(roomMorningTextureJpg)
    const roomNightTexture = useTexture(roomNightTextureJpg)
    const ytPatchMorningTexture = useTexture(ytPatchMorningJpg)
    const ytPatchNightTexture = useTexture(ytPatchNightJpg)
    ...
    const [
        ...
        const travMaterial = new THREE.MeshBasicMaterial({ map: travTexture })
        ...
        const libxdVideoMaterial = createVideoMaterial(weeblyVideoMp4)
        const xwingVideoMaterial = createVideoMaterial(supplexVideoMp4)
        const aeonsVideoMaterial = createVideoMaterial(gdocVideoMp4)

        // Overlay plane that covers the text strip on the YouTube button.
        // Blends between the user-supplied morning/night patch images using the same
        // uDayTimeMix as the room so colour matches perfectly at all times of day.
        const ytTextOverlayGeo = new THREE.PlaneGeometry(0.70, 0.46)

        fixTexture(ytPatchMorningTexture)
        fixTexture(ytPatchNightTexture)

        const ytTextOverlayMaterial = new THREE.ShaderMaterial({
            uniforms: {
                uDayTimeMix: roomMaterial.uniforms.uDayTimeMix,
                uTextureMorning: { value: ytPatchMorningTexture },
                uTextureNight: { value: ytPatchNightTexture }
            },
            vertexShader: roomVertexShader,
            fragmentShader: roomFragmentShader
        })

        for (const texture of [meTexture, travTexture, roomMorningTexture, roomNightTexture]) {
            fixTexture(texture)
        }
        // patch textures already fixed above


        for (const material of [meMaterial, travMaterial, roomMaterial, plantMaterial, libxdVideoMaterial, xwingVideoMaterial, aeonsVideoMaterial, ytTextOverlayMaterial]) {
    ...
    // Canvas texture: white text + YouTube logo drawn over the background patch
    const [ytLabelMaterial] = useMemo(() => {
        const CW = 1024, CH = 672
        ...
        const draw = (logoImg) => {
            ...
            // "Presented to" — small italic
            ctx.font = 'italic 38px Arial, sans-serif'
            ctx.fillText('Presented to', CW / 2, 200)

            // Channel name — large bold
            ctx.font = 'bold 78px Arial, sans-serif'
            ctx.fillText('Moltentungsten', CW / 2, 295)

            // "For passing 1,000,000 subscribers" — small
            ctx.font = '34px Arial, sans-serif'
            ctx.fillText('For passing 1,000,000 subscribers', CW / 2, 372)

            // YouTube logo — convert black bg → transparent via brightness-as-alpha
            if (logoImg) {
                ...
                const logoH = 88
                const logoW = tmp.width * (logoH / tmp.height)
                ctx.drawImage(tmp, CW / 2 - logoW / 2, 460 - logoH / 2, logoW, logoH)
            }
    ...
    return (
        <group>
            ...
            {/* Background colour patch — blends morning/night to hide original text */}
            <mesh
                geometry={ytTextOverlayGeo}
                material={ytTextOverlayMaterial}
                position={[1.8479, 0.308, -0.364]}
            />

            {/* "Presented to / Moltentungsten / For passing…" label on top */}
            <mesh
                geometry={ytTextOverlayGeo}
                material={ytLabelMaterial}
                position={[1.8479, 0.308, -0.363]}
            />
