...
import oswaldBoldUrl from '../../assets/fonts/Oswald-Bold.ttf?url'
...
const placeholderGeometry = new THREE.PlaneGeometry(0.001, 0.001)
const solidExpandGeometry = new THREE.PlaneGeometry(10, 10)
...
function Controls() {
    ...
    const [introTextGeometry, setIntroTextGeometry] = useState(null)

    useEffect(() => {
        const ttfLoader = new TTFLoader()
        const fontLoader = new FontLoader()

        ttfLoader.load(oswaldBoldUrl, (json) => {
            ...
            const geo1 = new TextGeometry("YASH'S", { font, size: topSize, height: depth, curveSegments: 12 })
            ...
            const geo2 = new TextGeometry('ROOM', { font, size: botSize, height: depth, curveSegments: 12 })
            geo2.computeBoundingBox()
            const w2 = geo2.boundingBox.max.x - geo2.boundingBox.min.x
            const h2 = geo2.boundingBox.max.y - geo2.boundingBox.min.y

            // Centre both words as a group at (0, 0)
            const gap = h1 * 0.12
            const totalHeight = h1 + gap + h2

            geo1.translate(-w1 / 2, totalHeight / 2 - h1, 0)
            geo2.translate(-w2 / 2, -totalHeight / 2, 0)

            const merged = mergeBufferGeometries([geo1, geo2])
            setIntroTextGeometry(merged)
        })
    }, [])
    ...
    return (
        <group>
            ...
            <PerspectiveCamera makeDefault fov={50} position={[0, 0, 6]}>
                <MotionMask 
                    id={1} 
                    geometry={experienceStarted ? solidExpandGeometry : (introTextGeometry || placeholderGeometry)}
