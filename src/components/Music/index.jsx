...
function Music({ fftSize = 64, barMargin = 3, barWidth = 1, amplitude = 15 }) {
    ...
    // Stable ended-event handler — always delegates to the latest switchTrack
    const switchTrackRef = useRef(null)
    const onTrackEnded = useRef(null)
    if (!onTrackEnded.current) {
        onTrackEnded.current = () => switchTrackRef.current?.()
    }

    // Connect an Audio element to the shared analyzer (safe to call multiple times)
    const connectToAnalyzer = (audio) => {
        ...
        try {
            ...
            // already connected to this AudioContext — skip
        }
    }

    // Always-up-to-date switch logic stored in a ref so ended handlers don't go stale
    switchTrackRef.current = () => {
        // Remove ended listener from the track that just finished / is being skipped
        const currentAudio = audioManager.get('music')
        currentAudio.removeEventListener('ended', onTrackEnded.current)

        const newTrack = audioManager.nextTrack()
        setCurrentTrackName(newTrack.name)

        const newAudio = audioManager.get('music')
        connectToAnalyzer(newAudio)
        newAudio.addEventListener('ended', onTrackEnded.current)

        // Always play immediately — setState alone won't re-fire the useEffect
        // if musicPlaying was already true
        useAppStore.setState({ musicPlaying: true })
        audioManager.play('music')
    }

    // Two.js frequency-bar setup
    useEffect(() => {
    ...
    // AudioContext init (once, when audio is first enabled)
    useEffect(() => {
        if (audioEnabled && !initializedRef.current) {
            ...
            music.addEventListener('ended', onTrackEnded.current)
    ...
    // Play / pause + frequency animation
    useEffect(() => {
        ...
        <S.Music
            ...
            <S.Controls
                ...
                <svg
                    ...
                    onMouseEnter={() => {
                        useAppStore.setState({ inHover: true })
                        useAppStore.setState({ cursorText: 'next' })
                    }}
                    onClick={(e) => {
                        e.stopPropagation()
                        switchTrackRef.current()
                    ...
                    <path d="M6 18l8.5-6L6 6v12zm2-8.14L11.03 12 8 14.14V9.86zM18 6h2v12h-2z" />
                </svg>
            </S.Controls>
